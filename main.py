...
OMDB_IMG_500 = "https://img.omdbapi.com/"
...
DF_PATH = os.path.join(BASE_DIR, "mv.pkl")
INDICES_PATH = os.path.join(BASE_DIR, "indices.pkl")
TFIDF_MATRIX_PATH = os.path.join(BASE_DIR, "tfidf_matrix.pkl")
TFIDF_PATH = os.path.join(BASE_DIR, "tfidf.pkl")

mv: Optional[pd.DataFrame] = None
...
class SearchBundleResponse(BaseModel):
    ...
    tdidf_recommendations: List[TFIDFRecItem]
...
async def omdb_movie_details(movie_id: int) -> OMDBMovieDetails:
...
def tdidf_recommend_titles(
    query_title: str, top_n: int = 10
) -> List[Tuple[str, float]]:
    """
    Returns list of (title, score) from local mv using cosine similarity on TF-IDF matrix.
    Safe against missing columns/rows.
    """
    global mv, tdidf_matrix
    if mv is None or tdidf_matrix is None:
    ...
    # query vector
    qv = tdidf_matrix[idx]
    scores = (tdidf_matrix @ qv.T).toarray().ravel()
    ...
    for i in order:
        if int(i) == int(idx):
            continue
        try:
            title_i = str(mv.iloc[int(i)]["title"])
        except Exception:
            continue
...
def load_pickles():
    global mv, indices_obj, tdidf_matrix, tdidf_obj, TITLE_TO_IDX
    ...
    with open(TFIDF_MATRIX_PATH, "rb") as f:
        tdidf_matrix = pickle.load(f)

    # Load tdidf vectorizer (optional, not used directly here)
    ...
    # sanity
    if mv is None or "title" not in mv.columns:
        raise RuntimeError("mv.pkl must contain a DataFrame with a 'title' column")
...
async def home(
    ...
    try:
        ...
        data = await omdb_get(f"/movie/{category}", {"language": "en-US", "page": 1})
        return await omdb_cards_from_results(data.get("results", []), limit=limit)
...
async def omdb_search(
    ...
    """
    Returns RAW OMDB shape with 'results' list.
...
@app.get("/movie/id/{omdb_id}", response_model=OMDBMovieDetails)
async def movie_details_route(omdb_id: int):
    return await omdb_movie_details(omdb_id)
...
async def recommend_genre(
    ...
    """
    Given a OMDB movie ID:
    - fetch details
    - pick first genre
    - discover movies in that genre (popular)
    """
    details = await omdb_movie_details(omdb_id)
    ...
    return [c for c in cards if c.omdb_id != omdb_id]


# ---------- TF-IDF ONLY (debug/useful) ----------
@app.get("/recommend/tdidf")
async def recommend_tdidf(
    title: str = Query(..., min_length=1),
    top_n: int = Query(10, ge=1, le=50),
):
    recs = tdidf_recommend_titles(title, top_n=top_n)
...
async def search_bundle(
    query: str = Query(..., min_length=1),
    tdidf_top_n: int = Query(12, ge=1, le=30),
    ...
    tdidf_items: List[TFIDFRecItem] = []

    recs: List[Tuple[str, float]] = []
    try:
        # try local dataset by OMDB title
        recs = tdidf_recommend_titles(details.title, top_n=tdidf_top_n)
    except Exception:
        # fallback to user query
        try:
            recs = tdidf_recommend_titles(query, top_n=tdidf_top_n)
        except Exception:
            recs = []

    for title, score in recs:
        card = await attach_omdb_card_by_title(title)
        tdidf_items.append(TFIDFRecItem(title=title, score=score, omdb=card))

    # 2) Genre recommendations (OMDB discover by first genre)
    ...
    if details.genres:
        ...
        genre_recs = [c for c in cards if c.omdb_id != details.omdb_id]

    return SearchBundleResponse(
        query=query,
        movie_details=details,
        tdidf_recommendations=tdidf_items,
